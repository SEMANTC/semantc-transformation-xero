...
SELECT DISTINCT
    ...
    SAFE_CAST(JSON_VALUE(journal_line, '$.created_date_utc') AS TIMESTAMP) AS journal_line_created_date_utc,
    SAFE_CAST(JSON_VALUE(journal_line, '$.updated_date_utc') AS TIMESTAMP) AS journal_line_updated_date_utc,
    ingestion_time
FROM 
