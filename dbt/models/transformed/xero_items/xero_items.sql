...
SELECT DISTINCT
    ...
    JSON_VALUE(payload, '$.inventory_asset_account_code') AS inventory_asset_account_code,
    JSON_VALUE(payload, '$.purchase_description') AS purchase_description,
    JSON_VALUE(payload, '$.sales_details.account_code') AS sales_details_account_code,
    JSON_VALUE(payload, '$.sales_details.cogs_account_code') AS sales_details_cogs_account_code,
    JSON_VALUE(payload, '$.sales_details.tax_type') AS sales_details_tax_type,
    SAFE_CAST(JSON_VALUE(payload, '$.sales_details.unit_price') AS FLOAT64) AS sales_details_unit_price,
    JSON_VALUE(payload, '$.purchase_details.account_code') AS purchase_details_account_code,
    JSON_VALUE(payload, '$.purchase_details.cogs_account_code') AS purchase_details_cogs_account_code,
    JSON_VALUE(payload, '$.purchase_details.tax_type') AS purchase_details_tax_type,
    SAFE_CAST(JSON_VALUE(payload, '$.purchase_details.unit_price') AS FLOAT64) AS purchase_details_unit_price,
